...
由 walls/platforms/ 的 per-platform handler 处理;这里是"没有专属 handler 匹配时"的 generic fallback。
[CONFIDENCE: CONFIRMED — verbatim 逻辑迁移,sync→async 适配].
...
async def fill_in(scope, names: list[str], val: str) -> bool:
    ...
    same label/placeholder targeting."""
    for n in names:
...
async def register_generic(scope, reg: dict, dbg: dict) -> bool:
    ...
    button (or link). Returns True if it clicked a submit control."""
