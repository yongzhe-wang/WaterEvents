...
export default function EventsView() {
  ...
  const [src, setSrc] = useState<{ id: string; title: string; url: string | null; content: string | null; loading: boolean } | null>(null);
  ...
  function openSource(e: EventRow) {
    setSrc({ id: e.id, title: e.title || (labelById[e.company_id] || ""), url: null, content: null, loading: true });
    fetch(`/api/page?event_id=${encodeURIComponent(e.id)}`).then((r) => r.json())
      .then((d) => setSrc({ id: e.id, title: e.title || (labelById[e.company_id] || ""), url: d.url || null, content: d.content ?? null, loading: false }))
      .catch(() => setSrc({ id: e.id, title: e.title || "", url: null, content: null, loading: false }));
  }
          ...
          <div className="src-modal" onClick={(ev) => ev.stopPropagation()}>
            ...
            <div className="src-body">
              {src.loading ? "Loading…"
                ...
                : "No source page stored for this event yet (the crawler writes page content on the next run)."}
            </div>
          </div>
        </div>
      )}
