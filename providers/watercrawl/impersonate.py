...
Upstream: watercrawl.render_full calls fetch() when the headless render comes back walled/thin. Downstream: the
(text, links) fold into the crawl link set exactly like a rendered page.
"""
...
def get(url: str, *, want_json: bool = False):
    ...
    try:
        ...
        if r is None:
            from .. import webshare            # lazy: the residential-proxy provider (env-configured)
            if webshare.enabled():
