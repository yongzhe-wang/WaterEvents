...
# SHRINKS the VLM's job to the three things ONLY a model can do: confirm metadata, parse speaker-attributed transcript,
# and pick which links are THIS event's media. The VLM still sees the full tagged page_text (INPUT, within the 32768
# budget) so it can route urls in context — but with no basic_info OUTPUT field it can NEVER overflow by copying tables.
...
SYSTEM_ROUTE = """You are given ONE web page belonging to a KNOWN investor-relations event (its rendered text with links \
inline as [anchor](Lnn) where Lnn is that link's short REFERENCE ID, e.g. L7). THIS PAGE'S READABLE CONTENT (paragraphs, \
headings, lists, financial TABLES) HAS ALREADY BEEN EXTRACTED SEPARATELY AND DETERMINISTICALLY — do NOT reproduce it, do \
NOT copy tables, do NOT summarize the body. Your job is ONLY the three things that need a model. Output ONE JSON object with:
...
3) "new_urls": the REFERENCE IDs (Lnn) of the links that are THIS EVENT'S OWN materials — its MEDIA and content, nothing \
else. Include ONLY: the webcast / audio (mp3) / VIDEO (mp4) / YouTube or webcast-replay link; the PDF / slide deck / \
presentation / transcript file(s) shown for THIS event; and any sub-page OF THIS event. \
Do NOT include the site's global navigation or menus, social-media links (Facebook / Twitter / YouTube channel / \
LinkedIn), legal / footer links (privacy, terms, accessibility, cookies, "Powered by"), or investor-relations utility \
pages (email alerts, RSS, stock quote & chart, governance, board, FAQs, download library, request info, contact) — \
those are the IR SITE's chrome, NOT this event's media. If this event has no media links on the page, return []. \
List EVERY qualifying id — do not stop short; the ids are cheap to emit so there is no reason to under-list.

Output STRICT JSON only, no prose: {"title":"","date":"","type":"","transcript_segments":[],"new_urls":[]}."""


# ROUTE schema — basic_info REMOVED (deterministic side owns it); only metadata + transcript + new_urls. Guided decoding
# on this schema CANNOT emit a body-copy field, so the output stays tiny (metadata + url ids + optional transcript) and
# the earnings-table overflow is impossible by construction. {DESIGN wf_7b61c8d0} [CONFIDENCE: CONFIRMED — no basic_info key].
SCHEMA_ROUTE = {
    "type": "object",
    "properties": {
        "title": {"type": "string"},
