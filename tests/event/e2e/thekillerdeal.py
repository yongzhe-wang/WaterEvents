...
import asyncpg                                                # read the FULL company universe from waterevents.companies (KD_ALL)

from agent.event_agent.crawl import crawl_company          # THE full pipeline: BFS render→extract→route→render
from agent.event_agent.storage import events as db                           # Postgres layer → events land in waterevents.events (frontend reads it)
...
KD_ALL = os.environ.get("KD_ALL", "") in ("1", "true", "yes")   # read the FULL 2785 universe from waterevents.companies, not sample_100
KD_URLS_FILE = os.environ.get("KD_URLS_FILE", "")          # JSON array of specific urls to crawl (e.g. a rerun of the low-count companies)
...
async def main() -> None:
    ...
    if KD_URLS_FILE:                                       # RERUN mode: crawl exactly the urls in this JSON file (e.g. the
        urls = json.load(open(KD_URLS_FILE, encoding="utf-8"))   # low-count companies, with their seed-replaced ir_urls)
    elif KD_ALL:                                           # FULL universe: every distinct ir_url in the canonical waterevents.companies
        # WHY waterevents (not public.companies): public 是 07-22 冻结的 legacy schema, 已整个 drop; canonical seed 宇宙
        # (完整 2785 家 + event_hubs)现在在 waterevents.companies。{SCAN whn86f4f7 "canonical=waterevents; public 是 legacy"}.
        conn = await asyncpg.connect(os.environ["WATEREVENTS_DB_DSN"], statement_cache_size=0)
        rows = await conn.fetch("SELECT DISTINCT ir_url FROM waterevents.companies "
