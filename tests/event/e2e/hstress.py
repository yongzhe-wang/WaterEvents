...
用一句话讲完: 从 waterevents.companies 拿全部 distinct ir_url → 每个 url 跑 crawl_company(max_pages=1) 只渲染那一页(不进
...
DSN = os.environ.get("WATEREVENTS_DB_DSN", "")              # source: waterevents.companies; only required in DB mode (not file mode)
...
async def _get_seeds() -> list[str]:
    ...
    c = await asyncpg.connect(DSN, statement_cache_size=0)   # canonical schema = waterevents (legacy public was dropped 2026-07-26)
    rows = await c.fetch("select distinct ir_url from waterevents.companies "
